...
class BraTS2019(Dataset):
    ...
    def __init__(self, meta_info=None, base_dir=None, split='train', num=None, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        # image = h5f['image'][:]
        # label = h5f['label'][:]
        image_name = self._base_dir + "image/{}-t2f.nii.gz".format(name)
        imgitk = sitk.ReadImage(image_name)
        image = sitk.GetArrayFromImage(imgitk)
        image = image.astype(np.float32)
        # image = image.astype(np.float32) / 255.0
        label_name = self._base_dir + "label/{}-seg.nii.gz".format(name)
        labelitk = sitk.ReadImage(label_name)
