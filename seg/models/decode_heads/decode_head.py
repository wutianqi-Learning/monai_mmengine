...
class BaseDecodeHead(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if sampler is not None:
            ...
            self.sampler = None
        # self.conv_seg = ConvModule(channels,
        #                            self.out_channels,
        #                            kernel_size=1,
        ...
        if conv_seg_3x3 is True:
            self.conv_seg = nn.Conv2d(channels, self.out_channels, kernel_size=3, padding=1)
        if dropout_ratio > 0:
    ...
    def _transform_inputs(self, inputs):
        ...
        if self.input_transform == 'resize_concat':
            ...
            upsampled_inputs = [
                resize(
                    input=x,
                    size=inputs[0].shape[1:] if self.resize_mode == 'trilinear' else inputs[0].shape[2:],
...
class LossHead(BaseModule, metaclass=ABCMeta):
    def __init__(self,
                 num_classes,
                 out_channels=None,
                 in_index=-1,
