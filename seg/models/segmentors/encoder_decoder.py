...
from torch.hub import download_url_to_file, _get_torch_home
...
class EncoderDecoder(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 backbone_load_url: Optional[str] = None,
        ...
        if backbone_load_url is not None:
            # assert hasattr(self.backbone, 'load_from')
            # parts = urlparse(backbone_load_url)
            # filename = os.path.basename(parts.path)
            # file_dir = os.path.join(_get_torch_home(), filename)
            # if not os.path.exists(file_dir):
            #     download_url_to_file(backbone_load_url, file_dir)
            # self.backbone.load_from(torch.load(file_dir))

            model_dict = load_url(backbone_load_url)["state_dict"]
