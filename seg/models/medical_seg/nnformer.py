...
class SwinTransformerBlock_kv(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if len(input_resolution) == 3:
            _window_size = to_3tuple(self.window_size)
        elif len(input_resolution) == 2:
            _window_size = to_2tuple(self.window_size)
        self.attn = WindowAttention_kv(
            dim, window_size=_window_size, num_heads=num_heads,
    ...
    def forward(self, x, mask_matrix, skip=None, x_up=None):
        ...
        if len(self.input_resolution) == 3:
            ...
            shifted_x = window_reverse(attn_windows, self.window_size, skip_shape)  # B H' W' C
...
class PatchMerging(nn.Module):
    ...
    def __init__(self, conv_op, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.reduction = conv_op(dim, dim * 2, kernel_size=3, stride=2, padding=1)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if downsample is not None:
            self.downsample = downsample(conv_op=conv_op, dim=dim, norm_layer=norm_layer)
        else:
    ...
    def forward(self, x, shape):
        ...
        if len(shape) == 3:
            ...
            for blk in self.blocks:
                x = blk(x, attn_mask, shape)
            if self.downsample is not None:
                x_down = self.downsample(x, shape)
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self,
        ...
        self.Upsample = upsample(conv_op=conv_op, dim=2 * dim, norm_layer=norm_layer)

    def forward(self, x, skip, shape):
...
class project(nn.Module):
    def __init__(self, conv_op, in_dim, out_dim, stride, padding, activate, norm, last=False):
        super().__init__()
        self.out_dim = out_dim
        self.conv1 = conv_op(in_dim, out_dim, kernel_size=3, stride=stride, padding=padding)
        self.conv2 = conv_op(out_dim, out_dim, kernel_size=3, stride=1, padding=1)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, conv_op, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        super().__init__()
        ...
        self.proj1 = project(conv_op, in_chans, embed_dim // 2, stride1, 1, nn.GELU, nn.LayerNorm, False)
        self.proj2 = project(conv_op, embed_dim // 2, embed_dim, stride2, 1, nn.GELU, nn.LayerNorm, True)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, out_shape, x, W_shape = layer(x, W_shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, skips):
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            x, x_shape = layer(x, skips[i], x_shape)
            if len(x_shape) == 3:
                S, H, W = x_shape
...
class final_patch_expanding(nn.Module):
    def __init__(self, conv_op, dim, num_class, patch_size):
        super().__init__()
        if conv_op == nn.Conv3d:
...
class nnFormer(nn.Module):
    ...
    def __init__(self, crop_size=[64, 128, 128],
        ...
        self.model_down = Encoder(pretrain_img_size=crop_size,
                                  ...
                                  conv_op=conv_op)
        self.decoder = Decoder(conv_op=conv_op, pretrain_img_size=crop_size, embed_dim=embed_dim, window_size=window_size[::-1][1:],
        ...
        if self.do_ds:
            ...
            for i in range(len(depths) - 1):
                self.final.append(final_patch_expanding(conv_op, embed_dim * 2 ** i, num_classes, patch_size=patch_size))

        else:
            self.final.append(final_patch_expanding(conv_op, embed_dim, num_classes, patch_size=patch_size))
