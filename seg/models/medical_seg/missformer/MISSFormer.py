# Copyright for MISSFormer: https://github.com/ZhifangDeng/MISSFormer/tree/main
import torch
...
class BridgeLayer_4(nn.Module):
    ...
    def forward(self, inputs):
        ...
        tem1 = tx[:, :128*128, :].reshape(B, -1, C)
        tem2 = tx[:, 128*128:128*128+64*64*2, :].reshape(B, -1, C * 2)
        tem3 = tx[:, 128*128+64*64*2:128*128+64*64*2+32*32*5, :].reshape(B, -1, C * 5)
        tem4 = tx[:, 128+64*64*2+32*32*5:128+64*64*2+32*32*5+16*16*8, :].reshape(B, -1, C * 8)

        m1f = self.mixffn1(tem1, 128, 128).reshape(B, -1, C)
        m2f = self.mixffn2(tem2, 64, 64).reshape(B, -1, C)
        m3f = self.mixffn3(tem3, 32, 32).reshape(B, -1, C)
        m4f = self.mixffn4(tem4, 16, 16).reshape(B, -1, C)
...
class BridgeLayer_3(nn.Module):
    ...
    def forward(self, inputs: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        tem2 = tx[:, :64*64*2, :].reshape(B, -1, C * 2)
        tem3 = tx[:, 64*64*2:64*64*2+32*32*5, :].reshape(B, -1, C * 5)
        tem4 = tx[:, 64*64*2+32*32*5:64*64*2+32*32*5+16*16*8, :].reshape(B, -1, C * 8)
...
class BridegeBlock_4(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk1 = bridge4[:, :128*128, :].reshape(B, 128, 128, C).permute(0, 3, 1, 2)
        sk2 = bridge4[:, 128*128:128*128+64*64*2, :].reshape(B, 64, 64, C * 2).permute(0, 3, 1, 2)
        sk3 = bridge4[:, 128*128+64*64*2:128*128+64*64*2+32*32*5, :].reshape(B, 32, 32, C * 5).permute(0, 3, 1, 2)
        sk4 = bridge4[:, 128*128+64*64*2+32*32*5:128*128+64*64*2+32*32*5+16*16*8, :].reshape(B, 16, 16, C * 8).permute(0, 3, 1, 2)
...
class BridegeBlock_3(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        sk2 = bridge4[:, :64*64*2, :].reshape(B, 64, 64, C * 2).permute(0, 3, 1, 2)
        sk3 = bridge4[:, 64*64*2:64*64*2+32*32*5, :].reshape(B, 32, 32, C * 5).permute(0, 3, 1, 2)
        sk4 = bridge4[:, 64*64*2+32*32*5:64*64*2+32*32*5+16*16*8, :].reshape(B, 16, 16, C * 8).permute(0, 3, 1, 2)
...
class MISSFormer(nn.Module):
    def __init__(self, num_classes=9, token_mlp_mode="mix_skip", encoder_pretrained=True):
        ...
        heads = [1, 2, 5, 8]
        d_base_feat_size = 16  # 16 for 512 inputsize   7for 224
        in_out_chan = [[32, 64], [144, 128], [288, 320], [512, 512]]

        dims, layers = [[64, 128, 320, 512], [2, 2, 2, 2]]
        self.backbone = MiT(512, dims, layers, token_mlp_mode)
