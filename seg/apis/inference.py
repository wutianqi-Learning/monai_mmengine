...
def _preprare_data(imgs: ImageType, model: BaseSegmentor, seg_dir: str):
...
def inference_model(model: BaseSegmentor,
                    img: ImageType,
                    seg_dir: str) -> Union[SegDataSample, SampleList]:
    ...
    """
    # prepare data
    data, is_batch = _preprare_data(img, model, seg_dir)
...
def show_result_pyplot(model: BaseSegmentor,
    ...
    visualizer.add_datasample(
        ...
        out_file=out_file,
        show=show)
    visualizer.set_image(mmcv.imrotate(visualizer.get_image(), 270))
    vis_img = visualizer.get_image()
    if draw_pred:
        visualizer.draw_texts(metric, positions=np.array([0, 0]), colors='w')
        vis_img = visualizer.get_image()
    mmcv.imwrite(mmcv.bgr2rgb(vis_img), out_file)
    return vis_img
