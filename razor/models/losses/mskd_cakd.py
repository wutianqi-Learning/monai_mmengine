...
class MSKDCAKDLoss(nn.Module):
    ...
    def forward(self, student_outputs, teacher_outputs, student_features, teacher_features):
        ...
        #                                       teacher_outputs[i + 4]))
        loss += 1.0 * (0.1 * self.CAKD(student_outputs, teacher_outputs)
                       + 0.2 * self.FNKD(student_outputs, teacher_outputs, student_features,
                                         teacher_features))
    ...
    def CAKD(self, student_outputs, teacher_outputs):
        ...
        # for b in range(1, B):
        #     Similarity_loss += F.cosine_similarity(student_outputs[b, :, :], teacher_outputs[0, :, :], dim=0)
