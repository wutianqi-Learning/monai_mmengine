...
class HausdorffDTLoss(_Loss):
    ...
    def forward(self, preds_S: torch.Tensor, preds_T: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        """
        Args:
        ...
        if self.to_onehot_y:
            if n_pred_ch == 1:
                ...
                target = preds_T.argmax(dim=1, keepdim=True)
                target = one_hot(target, num_classes=n_pred_ch)
        else:
            preds_T = torch.sigmoid(preds_T)
            preds_T = preds_T >= 0.5
            target = preds_T.float()
        ...
        if preds_T.shape != preds_S.shape and target.shape != preds_S.shape:
        ...
        # for i in k:
            ...
            pred_error_S = (ch_preds_S - ch_target) ** 2
...
class LogHausdorffDTLoss(HausdorffDTLoss):
    ...
    def forward(self, preds_S: torch.Tensor, preds_T: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        log_loss: torch.Tensor = torch.log(super().forward(preds_S, preds_T, target) + 1)
        self.loss_weight = loss_weight
