...
for p in patients:
    tgt_img_dir_p = os.path.join(tgt_img_dir, p)
    tgt_mask_dir_p = os.path.join(tgt_mask_dir, p)
    os.system('mkdir -p ' + tgt_img_dir_p)
    os.system('mkdir -p ' + tgt_mask_dir_p)
    ...
    for file_name in file_names:
        ...
        if 'Seg' in file_name:  # is a mask
            ...
            mask_path = os.path.join(tgt_mask_dir_p, tgt_name)
            mask = convert_label(img, convert_dict={0: 0, 255: 1})
            save_png_from_array(mask, mask_path)
        else:
            img_path = os.path.join(tgt_img_dir_p, tgt_name)
