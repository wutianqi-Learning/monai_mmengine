...
from seg.apis import inference_model, init_model, show_result_pyplot
...
def main():
    ...
    model = init_model(args.config, find_best_in_multi_run(args.checkpoint), device=args.device)
    if args.device == 'cpu':
    ...
    # data processing
    import mmcv
    image = mmcv.imread(args.img, 'grayscale')
    height, width = image.shape[0], image.shape[1]
    rgb_img = np.float32(np.array(Image.open(args.img).convert('RGB'))) / 255
